...
// app.getPath('userData') points to %APPDATA%/TARCRAFTLauncher (writable)
...
const GAME_DIR = path.join(USER_DATA_DIR, 'TARCRAFTClient');
const INSTANCE_DIR = GAME_DIR;
const INSTANCE_JAR = path.join(GAME_DIR, 'TARCRAFT 1.0.0.jar');
const INSTANCE_JSON = path.join(GAME_DIR, 'TARCRAFT 1.0.0.json');
const DEFAULT_MC_DIR = path.join(app.getPath('appData'), '.minecraft');
const VERSION_ID = 'TARCRAFT 1.0.0';
...
ipcMain.handle('launcher:launch', (_, payload) => {
    ...
    if (!fs.existsSync(versionJarPath)) {
        return { ok: false, msg: 'Не найден файл TARCRAFT 1.0.0.jar.' };
    }
    ...
    try {
        ...
        child.on('exit', (code, signal) => {
            ...
            if (mainWindow && !mainWindow.isDestroyed()) {
                mainWindow.show();
                mainWindow.focus();
                // Clear the status message when client closes
                mainWindow.webContents.send('client-closed');
                console.log('[Client Launch] Launcher window restored and status cleared');
            }
        });
        ...
        return { ok: true, msg: 'Клиент запускается...' };
    } catch (err) {
